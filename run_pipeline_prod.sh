...
# -----------------------------------------------------------------------------
# Script: run_pipeline_prod.sh
# Description:
#   This script runs the full Armadillo pipeline in the production environment. 
#   It uses the root folder /bioinformatics/Armadillo to create all folders.
#
#   It sets up the required directory structure, downloads input data from S3,
#   manages logs, and sends notifications via AWS SNS.
# 
#
...
echo "Checking the size of the file..." >> $run_logfile
...
date >> $run_logfile
echo "PhoeNix pipeline completed." >> $run_logfile



# Run Armadillo pipeline
python3 $basedir/armadillo_phoenix.py -r $1 -d $basedir -a $aws_bucket -c $refGenCatlog >> $run_logfile
conda deactivate
